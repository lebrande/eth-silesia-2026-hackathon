/**
 * Seed przykładowych wpisów FAQ dla backoffice.
 *
 * Użycie:
 *   npm run db:seed-faq
 *
 * Wymaga ustawionego DATABASE_URL w .env.local (Postgres z docker-compose).
 */
import path from "node:path";
import { config as loadEnv } from "dotenv";

// Next.js ładuje .env.local automatycznie, ale tsx/Node — nie. Musimy załadować
// zmienne PRZED zaimportowaniem lib/db (które inicjalizuje pg.Pool).
loadEnv({ path: path.resolve(process.cwd(), ".env") });
loadEnv({ path: path.resolve(process.cwd(), ".env.local"), override: true });

if (!process.env.DATABASE_URL) {
  console.error("✗ Brak DATABASE_URL. Sprawdź .env.local w katalogu backoffice-ai.");
  process.exit(1);
}
...
async function main() {
  // Importujemy dynamicznie, żeby moduły dostały już załadowane env.
  const [{ db }, { eq, sql }, { users, faqEntries }, bcryptMod] =
    await Promise.all([
      import("../lib/db"),
      import("drizzle-orm"),
